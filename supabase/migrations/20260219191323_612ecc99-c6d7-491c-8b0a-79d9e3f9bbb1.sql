-- Fix: Include valor_dotacao in saldo_disponivel and credito_nao_empenhado calculations
...
SELECT
  ...
  oa.valor_dotacao + COALESCE(cr.total_creditos, 0::numeric) - COALESCE(osc.total_consumo_os, 0::numeric) - COALESCE(emp.total_empenhos, 0::numeric) AS saldo_disponivel,
...
LEFT JOIN LATERAL (
  SELECT COALESCE(sum(os.valor_orcamento), 0::numeric) AS total_consumo_os
  FROM ordens_servico os
  WHERE os.regional_id = oa.regional_id
    AND os.status <> 'aberta'::os_status
    AND EXTRACT(year FROM os.data_abertura) = oa.exercicio::numeric
) osc ON true;