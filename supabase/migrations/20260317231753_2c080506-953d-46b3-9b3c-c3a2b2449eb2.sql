...
-- 1. Add motivo_bloqueio column to ordens_servico
ALTER TABLE public.ordens_servico
ADD COLUMN motivo_bloqueio text NULL;

-- Add comment for documentation
COMMENT ON COLUMN public.ordens_servico.motivo_bloqueio IS 'Motivo de bloqueio orçamentário: cota_regional_insuficiente, sem_cota_cadastrada, empenho_insuficiente, saldo_contrato_insuficiente, limite_modalidade_excedido';

-- 2. Update transition_os_status to handle motivo_bloqueio
...
BEGIN
  ...
  SET
    status = _new_status,
    updated_at = now(),
    -- Clear motivo_bloqueio when transitioning to execucao (budget resolved)
    motivo_bloqueio = CASE
      WHEN _new_status = 'execucao' THEN NULL
      WHEN _updates ? 'motivo_bloqueio' THEN _updates->>'motivo_bloqueio'
