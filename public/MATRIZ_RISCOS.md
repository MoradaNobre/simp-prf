...
| RS-02 | Segurança | Acesso não autorizado a documentos fiscais e fotografias de serviços | Configuração inadequada de políticas de acesso ao storage; URL de arquivo previsível ou sem expiração; compartilhamento indevido de links | Vazamento de documentos fiscais sensíveis; exposição de dados de fornecedores; violação de sigilo contratual | Baixa | Médio | **Baixo** | **[MITIGADO v1.8]** Bucket `os-fotos` configurado como privado; políticas RLS em `storage.objects` restringem acesso a usuários autenticados (INSERT, SELECT, UPDATE); todas as URLs de arquivos são geradas como signed URLs com expiração de 1 hora; utilitário `uploadToStorage` armazena apenas o path relativo no banco; hook `useSignedUrl` resolve URLs sob demanda; download ZIP utiliza signed URLs; compatibilidade retroativa com URLs públicas legadas via `extractPath` | Ausência de watermark em documentos sensíveis; sem registro de logs de acesso a documentos individuais | Considerar adição de watermark com identificação do usuário; implementar log de acesso a documentos sensíveis | Detectivo | Baixa |
| RS-03 | Segurança | Invocação não autenticada de Edge Functions críticas | Configuração `verify_jwt = false` nas Edge Functions do sistema; endpoints expostos publicamente | Envio de e-mails fraudulentos em nome do sistema | Baixa | Alto | **Médio** | **[MITIGADO PARCIALMENTE]** As Edge Functions críticas (`create-contract-user`, `delete-user`) implementam verificação de identidade via `getClaims(token)` e validação de role server-side. Funções de notificação (`notify-os-transition`, `notify-preposto`, `send-os-execucao`, `send-auth-email`) validam Authorization header. **[v1.7]** Edge Function `import-csv` removida do sistema. | Funções de notificação não verificam role do chamador (apenas autenticação); ausência de rate limiting; `verify_jwt = false` no config.toml significa que a proteção depende da implementação manual em cada função; ausência de validação de origem (CORS permissivo) | Implementar rate limiting nas Edge Functions; restringir CORS para domínios autorizados; adicionar verificação de role nas funções de notificação; considerar implementar API Gateway com throttling | Preventivo | Média |
...
| ID | Categoria | Evento de Risco | Causa Provável | Impacto Potencial | Probabilidade | Impacto | Nível de Risco | Controles Existentes | Vulnerabilidades Identificadas | Ação de Mitigação Recomendada | Tipo de Controle | Prioridade |
|----|-----------|----------------|----------------|-------------------|---------------|---------|----------------|---------------------|-------------------------------|-------------------------------|-----------------|------------|
| RT-01 | Tecnológico | Dependência de funções serverless (Edge Functions) para operações críticas | Concentração de lógica de negócio crítica em Edge Functions (criação de usuários, exclusão, notificações); ambiente serverless com limites de execução | Falha silenciosa em operações críticas; inconsistência entre estado do banco e estado esperado | Baixa | Médio | **Baixo** | **[MITIGADO]** 7 Edge Functions dedicadas com responsabilidades segregadas; validação de identidade (`getClaims`) e verificação de role server-side; Retry automático com backoff exponencial (até 2 retries, delays de 1s/2s/4s); Circuit breaker por função (abre após 5 falhas consecutivas, reabrir após 60s); Health check automatizado via cron job a cada 15 minutos (`check-function-health`); Dashboard de monitoramento com KPIs e gráficos; Alertas por e-mail via Resend; Tabela `edge_function_logs` com telemetria completa; Wrapper `monitoredInvoke` instrumentando todas as chamadas; **[v1.7]** Edge Function `import-csv` removida — funcionalidade de importação descontinuada, eliminando o risco de timeout por operações de longa duração | Nenhuma vulnerabilidade crítica remanescente | Manter monitoramento contínuo via dashboard e alertas | Detectivo | Baixa |
...
|---------|-----|----------------|------------------------------|
| **1º** | **RT-01** | Dependência de Edge Functions | ~~Crítico~~ → **Baixo**. Todos os mecanismos de resiliência implementados (retry, circuit breaker, monitoramento, alertas). Funcionalidade de importação CSV removida, eliminando o principal vetor de timeout. |
...
>
> **Nota v1.7:** O risco **RT-01** (Dependência de Edge Functions) foi reclassificado de **Médio** para **Baixo** após: implementação de retry com backoff, circuit breaker, health check automatizado (cron 15min), dashboard de monitoramento e remoção da Edge Function `import-csv` (eliminação do principal vetor de timeout).
>
> **Nota v1.8:** O risco **RS-02** (Acesso não autorizado a documentos) foi reclassificado de **Alto** para **Baixo** após: bucket tornado privado, políticas RLS em storage.objects e implementação de signed URLs com expiração de 1 hora.
...
1. ~~**Habilitar verificação JWT nas Edge Functions críticas**~~ ✅ **CONCLUÍDO** — As funções `create-contract-user` e `delete-user` utilizam `getClaims(token)` para validação de identidade e verificação de role server-side. Função `import-csv` removida do sistema (v1.7).
...
6. ~~**Implementar monitoramento de Edge Functions**~~ ✅ **CONCLUÍDO** — Dashboard de monitoramento, retry com backoff, circuit breaker, health check via cron (15min) e alertas por e-mail implementados (RT-01, ROP-01). Funcionalidade de importação CSV removida (v1.7).
7. ~~**Configurar políticas de acesso ao storage**~~ ✅ **CONCLUÍDO** — Bucket `os-fotos` tornado privado com RLS; signed URLs com expiração de 1h implementadas; compatibilidade retroativa mantida (RS-02).
...
| 1.6 | 28/02/2026 | Implementação de retry automático com backoff exponencial e circuit breaker no wrapper `monitoredInvoke`. Configuração de cron job para execução automática do `check-function-health` a cada 15 minutos via `pg_cron`. RT-01 reclassificado de Crítico → Médio. |
| 1.7 | 28/02/2026 | Remoção da Edge Function `import-csv` e funcionalidade de importação CSV/XLSX das páginas Gestão e Ativos. RT-01 reclassificado de Médio → Baixo (eliminação do principal vetor de timeout). RS-03 atualizado para refletir remoção. Contagem de Edge Functions atualizada para 7. Recomendação 5.2.2 concluída integralmente. |
| 1.8 | 28/02/2026 | Mitigação de RS-02 (Alto → Baixo): bucket `os-fotos` tornado privado; políticas RLS adicionadas em `storage.objects` (INSERT/SELECT/UPDATE para autenticados); signed URLs com expiração de 1h substituem URLs públicas; utilitário `uploadToStorage` armazena paths relativos; hook `useSignedUrl` para resolução sob demanda; download ZIP usa signed URLs; compatibilidade retroativa com URLs legadas. Recomendação 5.2.3 concluída. |
