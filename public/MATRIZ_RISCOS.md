...
| RS-03 | Segurança | Invocação não autenticada de Edge Functions críticas | Configuração `verify_jwt = false` em todas as 8 Edge Functions do sistema, incluindo `create-contract-user`, `delete-user` e `import-csv` | Criação de usuários não autorizados; exclusão maliciosa de usuários; importação de dados adulterados; envio de e-mails fraudulentos em nome do sistema | Média | Muito Alto | **Crítico** | Funções implementadas com validação interna parcial; acesso à URL das funções requer conhecimento do endpoint | Todas as Edge Functions configuradas com `verify_jwt = false` no `config.toml`; endpoints podem ser descobertos por scanning; ausência de rate limiting; não há validação de origem (CORS restritivo) | Habilitar `verify_jwt = true` para funções críticas (`create-contract-user`, `delete-user`, `import-csv`); implementar validação de service_role key para funções que requerem privilégio elevado; adicionar rate limiting; implementar validação de origem | Preventivo | Crítica |
...
| ID | Categoria | Evento de Risco | Causa Provável | Impacto Potencial | Probabilidade | Impacto | Nível de Risco | Controles Existentes | Vulnerabilidades Identificadas | Ação de Mitigação Recomendada | Tipo de Controle | Prioridade |
|----|-----------|----------------|----------------|-------------------|---------------|---------|----------------|---------------------|-------------------------------|-------------------------------|-----------------|------------|
| RC-01 | Conformidade | Insuficiência da trilha de auditoria para fins de prestação de contas | Tabela `audit_logs` não captura todas as operações relevantes; triggers de auditoria não cobrem todas as tabelas críticas; dados de auditoria insuficientes para reconstrução de eventos | Impossibilidade de rastreamento completo de operações financeiras; questionamento em processo de auditoria; fragilidade probatória | Média | Alto | **Alto** | Tabela `audit_logs` com campos action, table_name, record_id, old_data, new_data, user_id, description; triggers automáticos para eventos críticos; retenção indefinida de logs | Não há garantia de que todas as tabelas possuem triggers de auditoria; campo `user_id` pode ser nulo em operações de sistema; ausência de mecanismo de proteção contra adulteração dos logs de auditoria; `audit_logs` permite DELETE por gestor_master | Revisar e garantir triggers de auditoria em todas as tabelas com dados financeiros; tornar `audit_logs` append-only (remover política de DELETE); implementar assinatura digital nos registros de auditoria; criar relatório periódico de integridade dos logs | Detectivo / Preventivo | Alta |
...
| ID | Categoria | Evento de Risco | Causa Provável | Impacto Potencial | Probabilidade | Impacto | Nível de Risco | Controles Existentes | Vulnerabilidades Identificadas | Ação de Mitigação Recomendada | Tipo de Controle | Prioridade |
|----|-----------|----------------|----------------|-------------------|---------------|---------|----------------|---------------------|-------------------------------|-------------------------------|-----------------|------------|
| RT-01 | Tecnológico | Dependência excessiva de funções serverless (Edge Functions) para operações críticas | Concentração de lógica de negócio crítica em Edge Functions (criação de usuários, exclusão, notificações, importação CSV); ambiente serverless com limites de execução | Falha silenciosa em operações críticas; timeout em operações de longa duração (importação CSV); inconsistência entre estado do banco e estado esperado | Alta | Alto | **Crítico** | 8 Edge Functions dedicadas com responsabilidades segregadas; monitoramento via logs; funções com escopo definido | Todas as funções com `verify_jwt = false`; ausência de mecanismo de retry; timeout padrão do runtime serverless (tipicamente 30s); ausência de circuit breaker; não há monitoramento de saúde das funções; importação CSV pode exceder limites de memória/tempo | Implementar health check periódico para Edge Functions; adicionar retry com backoff exponencial; implementar circuit breaker para dependências externas (e-mail); migrar operações de longa duração (importação CSV) para processamento assíncrono com fila; adicionar telemetria | Preventivo / Detectivo | Crítica |
...
## 4. Top 5 Riscos Críticos

| Posição | ID | Evento de Risco | Justificativa da Criticidade |
|---------|-----|----------------|------------------------------|
| **1º** | **RS-03** | Invocação não autenticada de Edge Functions críticas | Todas as 8 Edge Functions operam sem verificação JWT. As funções `create-contract-user` e `delete-user` executam operações privilegiadas (criação e exclusão de usuários no sistema de autenticação) e podem ser invocadas por qualquer agente que conheça o endpoint. Este é o risco com maior potencial de exploração imediata e menor barreira de entrada para um atacante. |
| **2º** | **RS-01** | Escalonamento indevido de privilégios | A validação hierárquica de atribuição de perfis é implementada exclusivamente no frontend. Um usuário com conhecimento técnico poderia realizar chamadas diretas à API REST para atribuir perfis acima de sua hierarquia, comprometendo todo o modelo de segregação de acessos do sistema. |
| **3º** | **ROP-02** | Exclusão indevida de dados históricos | A ausência de soft delete em tabelas críticas (`ordens_servico`, `chamados`, `contratos`) significa que uma exclusão, mesmo acidental, resulta em perda irrecuperável de dados operacionais e financeiros. O impacto é amplificado pela natureza pública do órgão e pela necessidade de prestação de contas. |
| **4º** | **RT-01** | Dependência excessiva de Edge Functions | A concentração de operações críticas (criação de usuários, notificações, importação de dados) em ambiente serverless sem mecanismos de resiliência (retry, circuit breaker, monitoramento) cria um ponto de falha sistêmico que afeta múltiplos processos simultaneamente. |
| **5º** | **RE-01** | Dependência de desenvolvedor único | O risco de paralisação total da evolução e manutenção do sistema por concentração de conhecimento técnico em um único profissional é classificado como crítico pela combinação de alta probabilidade (fato estrutural) com impacto institucional severo. |
...
1. **Habilitar verificação JWT nas Edge Functions críticas** (`create-contract-user`, `delete-user`, `import-csv`), mitigando o risco RS-03 com impacto mínimo na operação.
...
| **Segurança de Infraestrutura** | Básico | HTTPS/TLS implementado, criptografia em repouso, mas Edge Functions sem autenticação, ausência de MFA, e storage sem políticas granulares de acesso. |
...
O SIMP-PRF demonstra maturidade técnica significativa em controle de acesso e integridade de dados, com implementação de RLS abrangente, segregação por perfil e regional, e mecanismos de bloqueio automático. Contudo, apresenta lacunas relevantes em segurança de infraestrutura (Edge Functions desprotegidas), continuidade de negócios (ausência de PCN e testes de restore), e governança formal (ausência de ato normativo de adoção).
...
A implementação das 15 recomendações estratégicas propostas elevaria o sistema ao nível **Avançado** de maturidade de gestão de riscos, compatível com as exigências de governança aplicáveis a sistemas de informação em órgãos da Administração Pública Federal.
